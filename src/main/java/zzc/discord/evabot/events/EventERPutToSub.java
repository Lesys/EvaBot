...
public class EventERPutToSub extends EventER {
	...
	public void executeCommand(@NotNull MessageReceivedEvent event) {
		...
		if (team != null) {
			ERPlayer player = team.getPlayerNames().stream().map(p -> ERPlayer.getERPlayer(p)).filter(p -> p.getDiscordName().equalsIgnoreCase(finalPlayerName)).findFirst().orElse(null);
